...
function transform() {
    ...
    const radiusX = -constant + x2 + (constY * y2);
    const radiusY = -constant + y2 + (constX * x2);
    ...
    if(coeffX !== 0 && coeffY !== 0){
        ...
        step4Container.textContent = `${x2}x² + ${y2} (y² ${coeffYSign} ${Math.abs(centerY)}y + ${constY}) = ${constant / -1} + ${x2} + ${constY}(${y2})`;
        //STEP 5
        aValue.textContent = `${x2}x²`;
        ...
        step4Container.textContent = `${x2} (x² ${coeffXSign} ${Math.abs(centerX)}x +${constX}) + ${y2}y² = ${constant / -1} + ${constX}(${x2}) + ${y2}`;
        //STEP 5
