...
function transform() {
    ...
    // Output the result
    const centerHide = document.querySelectorAll(".centerHide");
    const centerXInput = document.querySelector(".userCenterX");
    const centerYInput = document.querySelector(".userCenterY");
    const radiusInput = document.querySelector(".userRadius");
    const showAnswerParts = document.querySelector(".showAnswerParts");
    const yourAnswer = document.querySelector(".yourAnswer");
    const textResult = document.querySelector(".textResult");
    const finalCenterX = centerX / -1;
    const finalCenterY = centerY / -1;
    const finalRadius = Number.isInteger(Math.sqrt(radius)) ? Math.sqrt(radius) : Math.sqrt(radius).toFixed(2);
    centerValue.style.display = "none";
    radiusGraph.style.display = "none";
    centerHide.forEach(hide => hide.style.display = "block");
    centerValue.style.display = "";
    radiusGraph.style.display = "";
    yourAnswer.innerHTML = ``; 
    textResult.innerHTML = ``;
    centerValue.style.display = "";
    radiusGraph.style.display = "";
    showAnswerParts.addEventListener("click", () => {
        centerHide.forEach(hide => hide.style.display = "none");
        centerValue.style.display = "block";
        radiusGraph.style.display = "block";
        centerValue.innerHTML = `(${centerXInput.value}, ${centerYInput.value})`;
        radiusGraph.innerHTML = finalRadius;
        if(centerXInput.value == finalCenterX && centerYInput.value == finalCenterY && radiusInput.value == finalRadius){
            centerValue.style.display = "block";
            radiusGraph.style.display = "block";
            yourAnswer.innerHTML = `Your Answer: C: (${centerXInput.value}, ${centerYInput.value}), r = ${radiusInput.value}`;
            textResult.innerHTML = `Your answers are correct!`;
        } else {
            centerValue.style.display = "block";
            radiusGraph.style.display = "block";
            yourAnswer.innerHTML = `Your Answer: C: (${centerXInput.value}, ${centerYInput.value}), r = ${radiusInput.value}`;
            textResult.innerHTML = `Your answers are wrong!` 
        }
    });
...
if(coeffX !== 0 && coeffY !== 0) {
    step1Container.innerHTML = `x² ${coeffXSign} ${Math.abs((coeffX / xvalue).toFixed(2))}x ${yvalueSign} y² ${coeffYSign} ${Math.abs((coeffY / yvalue).toFixed(2))}y = ${constantDecimal}`;
