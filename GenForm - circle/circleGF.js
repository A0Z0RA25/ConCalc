...
function transform() {
    ...
    if(coeffX !== 0 && coeffY !== 0) {
        step1Container.textContent = `x² ${yvalueSign} y² ${coeffXSign} ${Math.abs(coeffX / xvalue)}x ${coeffYSign} ${Math.abs(coeffY / yvalue)}y = ${-constant}`;
        step2Container.textContent = `(x² ${coeffXSign} ${Math.abs(coeffX / xvalue)}x + ${xpow}) ${yvalueSign} (y² ${coeffYSign} ${Math.abs(coeffX / yvalue)}y + ${ypow}) = ${-constant}`;
        step3Container.textContent = `(x² ${coeffXSign} ${Math.abs(coeffX / xvalue)}x + ${xpow}) ${yvalueSign} (y² ${coeffYSign} ${Math.abs(coeffX / yvalue)}y + ${ypow}) = ${-constant} + ${xpow} + ${ypow}`;
        resultDiv.textContent = `(x ${coeffXSign} ${Math.abs(centerX)})² + (y ${coeffYSign} ${Math.abs(centerY)})² = ${radius}`;  
    } else if(coeffX == 0){
        step1Container.textContent = `x² ${yvalueSign} y² ${coeffYSign} ${Math.abs(coeffY)}y = ${-constant}`;
        step2Container.textContent = `x² ${yvalueSign} (y² ${coeffYSign} ${Math.abs(coeffY / yvalue)}y + ${ypow})² = ${-constant}`;
        step3Container.textContent = `x² ${yvalueSign} (y² ${coeffYSign} ${Math.abs(coeffY / yvalue)}y + ${ypow})² = ${-constant} + ${ypow}`;
        resultDiv.textContent = `x² + (y ${coeffYSign} ${Math.abs(centerY)})² = ${-constant + ypow}`;  
    } else {
        step1Container.textContent = `x² ${yvalueSign} y² ${coeffXSign} ${Math.abs(coeffX)} = ${-constant}`;
        step2Container.textContent = `(x² ${coeffXSign} ${Math.abs(coeffX / xvalue)}x + ${xpow}) ${yvalueSign} y² = ${-constant}`;
        step3Container.textContent = `(x² ${coeffXSign} ${Math.abs(coeffX / xvalue)}x + ${xpow}) ${yvalueSign} y² = ${-constant} + ${xpow}`;
        resultDiv.textContent = `(x ${coeffXSign} ${Math.abs(centerX)})² + y² = ${-constant + xpow}`;  
    } 
    }
} 
